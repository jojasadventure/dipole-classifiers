...
def define_and_parse_args():
    """Defines CLI arguments for the benchmarking script."""
    parser = argparse.ArgumentParser(description="Benchmark a dimension vector against the SST-2 dataset.")
...
def main():
    ...
    report_path = args.vector.parent / "sst2_benchmark_report.json"
    ...
    for r in correct_samples:
        print(f"{_Colors.GREEN}[CORRECT]{_Colors.ENDC} True: {r['true_label']:<8} | Pred: {r['predicted_label']:<8} | Score: {r['score']:+.4f} | Text: \"{r['text']}\"")

    print("\n--- Incorrect Classification Samples ---")
    if not incorrect_samples: print("None found.")
    for r in incorrect_samples:
        print(f"{_Colors.RED}[INCORRECT]{_Colors.ENDC} True: {r['true_label']:<8} | Pred: {r['predicted_label']:<8} | Score: {r['score']:+.4f} | Text: \"{r['text']}\"")
