...
def define_and_parse_args():
    parser = argparse.ArgumentParser(description="Benchmark a dimension vector against the GYAFC Formality dataset.")
...
def main():
    ...
    print(f"Dataset: GYAFC (family_relationships/test split)")
    ...
    try:
        ...
        # Pole A (Negative Score) = Informal, Pole B (Positive Score) = Formal
    ...
    for i in range(len(texts)):
        true_label_text = "Formal" if true_labels[i] == 1 else "Informal"
        pred_label_text = "Formal" if predicted_labels[i] == 1 else "Informal"
    ...
    report_path = args.vector.parent / "gyafc_benchmark_report.json"
